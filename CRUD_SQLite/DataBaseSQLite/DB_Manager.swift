...
class DB_Manager {
    ...
    private var photoUser: Expression<String>!
    ...
    init () {
        ...
        do {
            ...
            photoUser = Expression<String>("photoUser")
    ...
    public func addUser(nameValue: String, emailValue: String,phoneNumberValue: String, photoUserValue: String) {
        do {
            try db.run(users.insert(name <- nameValue, email <- emailValue, phoneNumber <- phoneNumberValue, photoUser <- photoUserValue))
        } catch {
            print(error.localizedDescription)
        }
    }
    ...
    public func getUsers() -> [UserModel] {
        ...
        do {
            ...
            for user in try db.prepare(users) {
                ...
                userModel.photoUser = user[photoUser]
                // append in new array
    ...
    public func getUser(idValue: Int64) -> UserModel {
        ...
        do {
            ...
            try user.forEach({ (rowValue) in
                ...
                userModel.photoUser = try rowValue.get(photoUser)
            })
        } catch {
            print(error.localizedDescription)
        }
    ...
    public func updateUser(idValue: Int64, nameValue: String, emailValue: String, phoneNumberValue: String, photoUserValue: String) {
        do {
            // get user using ID
            let user: Table = users.filter(id == idValue)
             
            // run the update query
            try db.run(user.update(name <- nameValue, email <- emailValue, phoneNumber <- phoneNumberValue, photoUser <- photoUserValue))
        } catch {
            print(error.localizedDescription)
        }
    }
